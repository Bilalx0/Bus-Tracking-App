const routes = [
  {
    id: 1,
    start: "Model Colony",
    end: "Dockyard",
    distance_km: 28,
    start_location: {
      latitude: 24.9069,
      longitude: 67.1386,
    },
    end_location: {
      latitude: 24.8081,
      longitude: 66.9893,
    },
    waypoints: [
      { latitude: 24.9069, longitude: 67.1386, sequence: 1 },
      { latitude: 24.91, longitude: 67.14, sequence: 2 },
      { latitude: 24.913, longitude: 67.145, sequence: 3 },
      { latitude: 24.915, longitude: 67.15, sequence: 4 },
      { latitude: 24.917, longitude: 67.155, sequence: 5 },
      { latitude: 24.92, longitude: 67.16, sequence: 6 },
      { latitude: 24.922, longitude: 67.165, sequence: 7 },
      { latitude: 24.924, longitude: 67.17, sequence: 8 },
      { latitude: 24.926, longitude: 67.175, sequence: 9 },
      { latitude: 24.928, longitude: 67.18, sequence: 10 },
      { latitude: 24.93, longitude: 67.185, sequence: 11 },
      { latitude: 24.932, longitude: 67.19, sequence: 12 },
      { latitude: 24.934, longitude: 67.195, sequence: 13 },
      { latitude: 24.936, longitude: 67.2, sequence: 14 },
      { latitude: 24.938, longitude: 67.205, sequence: 15 },
      { latitude: 24.94, longitude: 67.21, sequence: 16 },
      { latitude: 24.942, longitude: 67.215, sequence: 17 },
      { latitude: 24.944, longitude: 67.22, sequence: 18 },
      { latitude: 24.946, longitude: 67.225, sequence: 19 },
      { latitude: 24.833093, longitude: 66.971648, sequence: 20 },
    ],
  },
  {
    id: 2,
    start: "Power House",
    end: "Indus Hospital",
    distance_km: 30,
    start_location: {
      latitude: 24.985275,
      longitude: 67.065622,
    },
    end_location: {
      latitude: 24.81769,
      longitude: 67.111288,
    },
    waypoints: [
      {
        sequence: 1,
        name: "Nagan Chowrangi",
        latitude: 24.966344, 
        longitude: 67.066654
      },
      {
        sequence: 2,
        name: "Shafiq Mill",
        latitude: 24.956898,
        longitude: 67.076438,
      },

      {
        sequence: 3,
        name: "Sohrab Goth",
        latitude: 24.944536,
        longitude: 667.083883,
      },
      {
        sequence: 4,
        name: "Gulshan Chowrangi",
        latitude: 24.923873,
        longitude: 67.092237,
      },
      {
        sequence: 5,
        name: "NIPA",
        latitude: 24.917498,
        longitude: 67.097396,
      },
      {
        sequence: 6,
        name: "Johar Morr",
        latitude: 24.903896,
        longitude: 67.11385,
      },
      {
        sequence: 7,
        name: "COD",
        latitude: 24.891477,
        longitude: 67.122304,
      },
      {
        sequence: 8,
        name: "Drigh Road Station",
        latitude: 24.887158,
        longitude: 67.12814,
      },
      {
        sequence: 9,
        name: "Colony Gate",
        latitude: 24.887016,
        longitude: 67.15029,
      },
      {
        sequence: 10,
        name: "Shah Faisal Colony",
        latitude: 24.878769,
        longitude: 67.145406,
      },
      {
        sequence: 11,
        name: "Singer Chowrangi",
        latitude: 24.846993,
        longitude: 67.159911,
      },
      {
        sequence: 12,
        name: "Landhi Road",
        latitude: 24.823433,
        longitude: 67.139892,
      },
    ],
  },
  {
    id: 3,
    start: "Power House",
    end: "Shaan Chowrangi",
    distance_km: 31,
    start_location: {
      latitude: 24.985275,
      longitude: 67.065622,
    },
    end_location: {
      latitude: 24.9223,
      longitude: 67.0925,
    },
    waypoints: [
      {
        sequence: 1,
        name: "Power House",
        latitude: 24.984,
        longitude: 67.0661,
      },
      { 
        sequence: 2, 
        name: "UP More", 
        latitude: 24.973551,
        longitude:  67.066575
        
       },
      {
        sequence: 3,
        name: "Nagan Chowrangi",
        latitude: 24.966344, 
        longitude: 67.066654
      },
      { 
        sequence: 4, 
        name: "Sakhi Hasan", 
        latitude: 24.953895,
        longitude:  67.057620
        
       },
      {
        sequence: 5,
        name: "5 Star Chowrangi",
        latitude: 24.942835,
        longitude:  67.047815
      },
      {
        sequence: 6,
        name: "KDA Chowrangi",
        latitude: 24.9194,
        longitude: 67.0604,
      },
      {
        sequence: 7,
        name: "Nazimabad Eid Gah Ground",
        latitude: 24.955,
        longitude: 67.0624,
      },
      {
        sequence: 8,
        name: "Liaquatabad 10 Number",
        latitude: 24.9335,
        longitude: 67.0469,
      },
      {
        sequence: 9,
        name: "Essa Nagri",
        latitude: 24.9495,
        longitude: 67.0582,
      },
      {
        sequence: 10,
        name: "Civic Centre",
        latitude: 24.867,
        longitude: 67.0461,
      },
      {
        sequence: 11,
        name: "National Stadium",
        latitude: 24.8494,
        longitude: 67.0353,
      },
      { 
        sequence: 12, 
        name: "Karsaz", 
        latitude: 24.8606, 
        longitude: 67.067 

      },
      { 
        sequence: 13, 
        name: "Nursery", 
        latitude: 24.9023, 
        longitude: 67.067 

      },
      { 
        sequence: 14, 
        name: "FTC", 
        latitude: 24.871, 
        longitude: 67.078 

      },
      {
        sequence: 15,
        name: "Korangi Road",
        latitude: 24.8397,
        longitude: 67.1419,
      },
      {
        sequence: 16,
        name: "KPT Interchange",
        latitude: 24.8141,
        longitude: 67.0426,
      },
      {
        sequence: 17,
        name: "Shan Chowrangi",
        latitude: 24.8415,
        longitude: 67.1411,
      },
    ],
  },
  {
    id: 4,
    start: "Power House",
    end: "Tower",
    distance_km: 21,
    start_location: {
      latitude: 24.985275,
      longitude: 67.065622,
    },
    end_location: {
      latitude: 24.85,
      longitude: 67.0113,
    },

    waypoints: [
      {
        sequence: 1,
        name: "Nagan Chowrangi",
        latitude: 24.9645,
        longitude: 67.0671,
      },
      {
        sequence: 2,
        name: "Shafiq Mill",
        latitude: 24.956898,
        longitude: 67.076438,
      },

      {
        sequence: 3,
        name: "Sohrab Goth",
        latitude: 24.944536,
        longitude: 667.083883,
      },
      {
        sequence: 6,
        name: "Shahrah e Pakistan",
        latitude: 24.93368,
        longitude: 67.072072,
      },
      {
        sequence: 7,
        name: "Ayesha Manzil",
        latitude: 24.927569,
        longitude: 67.064452,
      },
      {
        sequence: 8,
        name: "Liaquatabad 10",
        latitude: 24.909217,
        longitude: 67.049298,
      },

      {
        sequence: 10,
        name: "Teen Hati",
        latitude: 24.893133,
        longitude: 67.043713,
      },
      {
        sequence: 11,
        name: "Jehangir Road",
        latitude: 24.886505,
        longitude: 67.041439,
      },
      {
        sequence: 12,
        name: "Numaish",
        latitude: 24.873272,
        longitude: 67.036121,
      },
      {
        sequence: 13,
        name: "Mobile Market",
        latitude: 24.864621,
        longitude: 67.024182,
      },
      {
        sequence: 14,
        name: "Urdu Bazar",
        latitude: 24.860795,
        longitude: 67.017739,
      },
      {
        sequence: 15,
        name: "Civil Hospital",
        latitude: 24.858373,
        longitude: 67.013403,
      },
      {
        sequence: 16,
        name: "Light House",
        latitude: 24.855988,
        longitude: 67.009472,
      },
      {
        sequence: 17,
        name: "City Court",
        latitude: 24.854359,
        longitude: 67.0067,
      },

      {
        sequence: 18,
        name: "Bolton Market",
        latitude: 24.852,
        longitude: 67.002502,
      },
      {
        sequence: 19,
        name: "Tower",
        latitude: 24.847876,
        longitude: 66.995209,
      },
    ],
  }, //9 ana hai
  {
    id: 9,
    start: "Gulshan e Hadeed",
    end: "Tower",
    distance_km: 42,
    start_location: { 
      latitude: 24.871035,
      longitude:  67.372195,
    },
    end_location: {  
      latitude: 24.846993,
      longitude: 66.993904,
    },
    
    waypoints: [
      { "sequence": 1, "latitude": 24.864258,  "longitude": 67.349962 },
      { "sequence": 2, "latitude":  24.860168, "longitude":  67.334441 },
      { "sequence": 3, "latitude": 24.859249, "longitude":  67.272122 }, 
      { "sequence": 4, "latitude": 24.854074,"longitude":  67.244323}, 
      { "sequence": 5, "latitude": 24.858811, "longitude": 67.208417},  
      { "sequence": 6, "latitude":24.869619, "longitude": 67.200188},  
      { "sequence": 7, "latitude": 24.884510, "longitude":  67.175418},  
      { "sequence": 8, "latitude": 24.887104, "longitude":  67.130080 }, 
      { "sequence": 9, "latitude": 24.884176, "longitude": 67.116339 }, 
      { "sequence": 10, "latitude": 24.876553, "longitude":   67.101895},

      { "sequence": 11, "latitude": 24.873596,  "longitude":  67.093605},
      { "sequence": 12, "latitude": 24.869579,"longitude": 67.087442 }, 
      { "sequence": 13, "latitude": 24.865899, "longitude":   67.079926},
      { "sequence": 14, "latitude":24.861293, "longitude": 67.067092}, 
      { "sequence": 15, "latitude":24.859234,  "longitude":67.052472}, 
      { "sequence": 16, "latitude": 24.855335,"longitude":  67.038675 },
      { "sequence": 17, "latitude": 24.849693, "longitude": 67.030386},  
      { "sequence": 18, "latitude": 24.853717, "longitude": 67.026373}, 
      { "sequence": 19, "latitude": 24.849999, "longitude": 67.010076 },
      { "sequence": 20, "latitude": 24.846993, "longitude": 66.993904 } 

    ]
  
  },
  {
    id: 10,
    start: "Numaish Chowrangi",
    end: "Ibrahim Hyderi",
    distance_km: 28,
    start_location: {
      latitude: 24.872171,
      longitude: 67.035674,
    },
    end_location: {
      latitude: 24.797568,
      longitude: 67.135636,
    },

    waypoints: [
      {
        sequence: 1,
        name: "Numaish",
        latitude: 24.872171,
        longitude: 67.035674,
      },
      {
        sequence: 2,
        name: "Taj Complex",
        latitude: 24.868394,
        longitude: 67.030361,
      },
      {
        sequence: 3,
        name: "Capri Cinema",
        latitude: 24.867739,
        longitude: 67.029259,
      },
      {
        sequence: 4,
        name: "Mobile Market",
        latitude: 24.863384,
        longitude: 67.025246,
      },
      { sequence: 5, 
        name: "Regal", 
        latitude: 24.860571, 
        longitude: 67.026124 
      },
      {
        sequence: 6,
        name: "Zainab Market",
        latitude: 24.855534,
        longitude: 67.028183,
      },
      {
        sequence: 7,
        name: "Abdullah Haroon",
        latitude: 24.853985,
        longitude: 67.028543,
      },

      {
        sequence: 8,
        name: "Fawara Chowk",
        latitude: 24.859,
        longitude: 67.0271,
      },
      {
        sequence: 9,
        name: "Metropole",
        latitude: 24.850477,
        longitude: 67.029957,
      },
      {
        sequence: 10,
        name: "Frere Hall",
        latitude: 24.847009,
        longitude: 67.031512,
      },
      {
        sequence: 11,
        name: "3 Talwar",
        latitude: 24.834058,
        longitude: 67.033711,
      },
      {
        sequence: 12,
        name: "Ocean Mall",
        latitude: 24.824084,
        longitude: 67.035231,
      },
      {
        sequence: 13,
        name: "2 Talwar",
        latitude: 24.821426,
        longitude: 67.03437,
      },
      {
        sequence: 14,
        name: "Abdullah Shah Ghazi Mazar",
        latitude: 24.809997,

        longitude: 67.031587,
      },

      {
        sequence: 16,
        name: "Dolmen Mall Clifton",
        latitude: 24.802896,
        longitude: 67.030411,
      },
      {
        sequence: 17,
        name: "Seaview Clock Tower",
        latitude: 24.77849,
        longitude: 67.054481,
      },
      {
        sequence: 18,
        name: "Khayaban e Ittehad",
        latitude: 24.807196,
        longitude: 67.077049,
      },
      {
        sequence: 19,
        name: "DHA Phase 7",
        latitude: 24.822477,
        longitude: 67.075054,
      },

      {
        sequence: 22,
        name: "KPT Interchange",
        latitude: 24.82966,
        longitude: 67.079645,
      },
      {
        sequence: 23,
        name: "Korangi Crossing",
        latitude: 24.815699,
        longitude: 67.110133,
      },
      {
        sequence: 25,
        name: "CBM University",
        latitude: 24.812166,
        longitude: 67.11705,
      },
      {
        sequence: 26,
        name: "PARCO",
        latitude: 24.801023,
        longitude: 67.133679,
      },
      {
        sequence: 27,
        name: "Ibrahim Hyderi",
        latitude: 24.797489,
        longitude: 67.135638,
      },
    ],
  },
  
  {
    id: 11,
    start: "Shireen Jinnah Colony", 
    end: "Miran Nakka Lyari",
    distance_km: 19,
    start_location: {
      latitude:24.8172752,  
      longitude: 67.0071251,
    },
    end_location: {
      latitude: 24.878276,  
      longitude: 66.995147,
    },
    waypoints: [
      { "sequence": 1, "latitude": 24.817107,  "longitude": 67.015543 },
      { "sequence": 2, "latitude":  24.815229, "longitude":  67.021209}, 
      { "sequence": 3, "latitude": 24.813011, "longitude":  67.027805}, 
      { "sequence": 4, "latitude":24.809143,"longitude":  67.034351}, 
      { "sequence": 5, "latitude": 24.805423, "longitude": 67.040354},  
      { "sequence": 6, "latitude":24.812640, "longitude":   67.047289}, 
      { "sequence": 7, "latitude":  24.816828,"longitude":  67.051319},  
      { "sequence": 8, "latitude": 24.819784, "longitude":  67.047677 }, 
      { "sequence": 9, "latitude": 24.824053, "longitude": 67.044553},  
      { "sequence": 10, "latitude": 24.829756, "longitude":    67.041225},

      { "sequence": 11, "latitude": 24.835425,  "longitude":  67.037237}, 
      { "sequence": 12, "latitude": 24.841348,"longitude": 67.032253}, 
      { "sequence": 13, "latitude": 24.846713,  "longitude":   67.024883},
      { "sequence": 14, "latitude":24.843135, "longitude": 67.011805},  
      { "sequence": 15, "latitude":24.845110,  "longitude":66.998538},  
      { "sequence": 16, "latitude": 24.852196,"longitude":  66.987981 }, 
      { "sequence": 17, "latitude":24.861798, "longitude": 66.987074},  
      { "sequence": 18, "latitude": 24.867333,  "longitude": 66.981986},
      { "sequence": 19, "latitude": 24.873815, "longitude":  66.982412 },
      { "sequence": 20, "latitude": 24.877492, "longitude": 66.990998 },
      { "sequence": 21, "latitude": 24.878276, "longitude": 66.995147 }  


    ]
  },
  {
    id: 12,
    start: "Khokrapar",
    end: "Lucky Star Saddar",
    distance_km: 32,
    start_location: {
      latitude: 24.8992,
      longitude: 67.1838,
    },
    end_location: {
      latitude: 24.8551,
      longitude: 67.0342,
    },
    waypoints: [
      {
        sequence: 1,
        name: "Naddi Kinara",
        latitude: 24.8729,
        longitude: 67.1052,
      },
      {
        sequence: 2,
        name: "Khokhrapar",
        latitude: 24.866,
        longitude: 67.1233,
      },
      {
        sequence: 3,
        name: "Saudabad Chowrangi",
        latitude: 24.8645,
        longitude: 67.1473,
      },
      {
        sequence: 4,
        name: "RCD Ground",
        latitude: 24.8282,
        longitude: 67.149,
      },
      {
        sequence: 5,
        name: "Kalaboard",
        latitude: 24.865,
        longitude: 67.1435,
      },
      {
        sequence: 6,
        name: "Malir 15",
        latitude: 24.8689,
        longitude: 67.1514,
      },
      {
        sequence: 7,
        name: "Malir Mandir",
        latitude: 24.8584,
        longitude: 67.1568,
      },
      {
        sequence: 8,
        name: "Malir Session Court",
        latitude: 24.8561,
        longitude: 67.1582,
      },
      {
        sequence: 9,
        name: "Murghi Khana",
        latitude: 24.8493,
        longitude: 67.169,
      },
      {
        sequence: 10,
        name: "Quaidabad",
        latitude: 24.8277,
        longitude: 67.1516,
      },
      {
        sequence: 11,
        name: "Dawood Chowrangi",
        latitude: 24.8362,
        longitude: 67.1609,
      },
      {
        sequence: 12,
        name: "Babar Market",
        latitude: 24.8482,
        longitude: 67.1619,
      },
      {
        sequence: 13,
        name: "Landhi Road",
        latitude: 24.827,
        longitude: 67.175,
      },
      {
        sequence: 14,
        name: "Nasir Jump",
        latitude: 24.842,
        longitude: 67.1763,
      },
      {
        sequence: 15,
        name: "Indus Hospital",
        latitude: 24.861,
        longitude: 67.1403,
      },
      {
        sequence: 16,
        name: "Korangi Crossing",
        latitude: 24.8324,
        longitude: 67.1582,
      },
      {
        sequence: 17,
        name: "Qayyumabad",
        latitude: 24.8289,
        longitude: 67.166,
      },
      {
        sequence: 18,
        name: "Defence Mor",
        latitude: 24.8069,
        longitude: 67.0562,
      },
      {
        sequence: 19,
        name: "National Medical Center",
        latitude: 24.8328,
        longitude: 67.0653,
      },
      {
        sequence: 20,
        name: "Gora Qabristan",
        latitude: 24.806,
        longitude: 67.0472,
      },
      {
        sequence: 21,
        name: "FTC",
        latitude: 24.8614,
        longitude: 67.0322,
      },
      {
        sequence: 22,
        name: "Jutt Land",
        latitude: 24.8276,
        longitude: 67.1432,
      },
      {
        sequence: 23,
        name: "Lines Area",
        latitude: 24.8612,
        longitude: 67.0212,
      },
      {
        sequence: 24,
        name: "Army Public School",
        latitude: 24.8651,
        longitude: 67.0241,
      },
      {
        sequence: 25,
        name: "Lucky Star Saddar",
        latitude: 24.8602,
        longitude: 67.0217,
      },
    ],
  },

                         //EV Bus route
  {
    //Ev-1
    id: 13,
    start: "CMH Malir",
    end: "Abdullah Shah Ghazi",
    distance_km: 41,
    start_location: { 
      latitude: 24.950289,
      longitude: 67.215504,
    },
    end_location: { 
      latitude:24.810337,
      longitude: 67.031572,
    },
    waypoints: [
      { latitude: 24.931027, longitude:  67.204651, sequence: 1 },
      { latitude: 24.913417, longitude: 67.198537, sequence: 2 }, 
      { latitude: 24.905300, longitude:  67.182483, sequence: 3 },
      { latitude: 24.897460,longitude:67.179809, sequence: 4 }, 
      { latitude: 24.891631, longitude: 67.168515, sequence: 5 }, 
      { latitude: 24.886872, longitude: 67.155828, sequence: 6 }, 
      { latitude: 24.880592, longitude:  67.109657, sequence: 7 },
      { latitude: 24.860172, longitude: 67.062528, sequence: 8 }, 
      { latitude: 24.850146, longitude:  67.053656, sequence: 9 },
      { latitude: 24.836069,  longitude: 67.069711, sequence: 10 },

      { latitude: 24.832792, longitude: 67.073847, sequence: 11 }, 
      { latitude: 24.822331, longitude: 67.075180, sequence: 12 }, 
      { latitude: 24.809754, longitude: 67.076830, sequence: 13 }, 
      { latitude: 24.798603, longitude: 67.073905, sequence: 14 }, 
      { latitude: 24.790789,longitude: 67.066415, sequence: 15 }, 
      { latitude: 24.778500, longitude:  67.054541, sequence: 16 },
      { latitude: 24.798488,longitude: 67.036176, sequence: 17 }, 
      { latitude: 24.810337, longitude: 67.031572, sequence: 18 },//
     
    ],
  },
  {     
    //EV-3
    id: 15,
    start: "Malir CP-5",
    end: "Numaish",
    distance_km: 41,
    start_location: {  
      latitude: 24.946711,
      longitude: 67.181477,
    },
    end_location: { 
      latitude:24.872538,
      longitude: 67.035936,
    },
    waypoints: [
      { latitude: 24.946711, longitude:  67.181477, sequence: 1 },
      { latitude: 24.942478, longitude: 67.172319, sequence: 2 },  
      { latitude: 24.938757, longitude:   67.154151, sequence: 3 },
      { latitude: 24.935678,longitude: 67.138437, sequence: 4 }, 
      { latitude: 24.921253, longitude: 67.135331, sequence: 5 },  
      { latitude: 24.909752, longitude: 67.122299, sequence: 6 }, 
      { latitude:24.904211,  longitude:  67.113927, sequence: 7 },
      { latitude: 24.901109, longitude: 67.112392, sequence: 8 },  
      { latitude: 24.898027, longitude:  67.104000, sequence: 9 }, 
      { latitude: 24.894147,   longitude: 67.088113, sequence: 10 },
      { latitude:  24.889617,  longitude: 67.067670, sequence: 11 },
      { latitude: 24.886569, longitude:  67.058265, sequence: 12 }, 
      { latitude: 24.879356, longitude: 67.047592, sequence: 13 },  
      { latitude:24.876990, longitude: 67.043055, sequence: 14 }, 
      { latitude: 24.872538, longitude:  67.035936, sequence: 15 }, 
    ],
  },
];

export function getRoutes() {
  return routes;
}

/*
  export function initializeBus(busId, startCoords, stopCoords){
      busData[busId] = {
          startingPoint: startCoords,
          endingPoint: stopCoords,
          currentLocation: startCoords
      }
  }
  
  export function updateBusData(busId, updatedProperty) {
    const bData = busData[busId]; // Accessing the data of the specific bus.
    busData[busId] = { ...bData, updatedProperty };
  }
    */
